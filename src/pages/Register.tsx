  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: { company_name: form.company, full_name: form.fullName },
      },
    });
    setLoading(false);
    if (error) return toast.error(error.message);
    toast.success("Account created — welcome to FieldPro!");
    navigate("/dashboard");
  };
    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-center px-6 py-12 sm:px-12 lg:px-20">
        <div className="mx-auto w-full max-w-sm">
          ...
          <h1 className="mt-10 text-3xl font-bold">Start your free trial</h1>
          <p className="mt-2 text-sm text-muted-foreground">14 days free. No credit card required.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input id="company" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} required className="mt-1.5" placeholder="R&R Plumbing" />
            </div>
            <div>
              <Label htmlFor="fullName">Your name</Label>
              <Input id="fullName" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} required className="mt-1.5" />
            </div>
            <div>
